...
FindQPath5(spot) {             ; PATH 5  DIAG   UP/LEFT
  ...
  while ( (col > 1) and (row < 8) ) {
    ...
    color := SqStat(spot)
    GoSpot(spot)
    p5[n] := { col: col, row: row, file: file, rank: row, spot: spot, color: color }
    if (color = my_color) {   ; collision same color
      return p5
    }
  }
...
FindQPath6(spot) {             ; PATH 6  DIAG  UP/RIGHT
  ...
  while ( (col < 8) and (row < 8) ) {
    ...
    color := SqStat(spot)
    GoSpot(spot)
    p6[n] := { col: col, row: row, file: file, rank: row, spot: spot, color: color }
    Sleep, 10
    if (color = my_color) {   ; collision same color
      return p6
    }
...
FindQPath7(spot) {             ; PATH 7  DIAG   DOWN/RIGHT
  ...
  while ( (col < 8) and (row > 1) ) {
    ...
    color := SqStat(spot)
    GoSpot(spot)
    p7[n] := { col: col, row: row, file: file, rank: row, spot: spot, color: color }
    Sleep, 10
    if (color = my_color) {   ; collision same color
      return p7
    }
...
FindQPath8(spot) {             ; PATH 8   DIAG  DOWN/LEFT
  ...
  while ( (col > 1) and (row > 1) ) {
    ...
    color := SqStat(spot)
    GoSpot(spot)
    p8[n] := { col: col, row: row, file: file, rank: row, spot: spot, color: color }
    Sleep, 10
    if (color = my_color) {   ; collision same color
      return p8
    }
